...
describe("外部公開ユースケース", () => {
  it("内部査読受理済みの資源評価を外部査読中に変更する", () => {
    const 内部査読受理済み = create内部査読受理済みの資源評価();
    const 管理者 = create認証済み資源評価管理者();

    const result = 外部公開ユースケース(内部査読受理済み, 管理者, テスト用バージョン);

    expect(result.外部査読中資源評価.作業ステータス).toBe("外部査読中");
    expect(result.外部公開済み.変化前).toBe("外部公開可能");
...
describe("再検討依頼取り消しユースケース", () => {
  describe("内部査読中への取り消し", () => {
    it("再検討中の資源評価を内部査読中に戻す（元ステータスから自動判定）", () => {
      const 再検討中 = create再検討中の資源評価_内部査読中から();
      const 副担当者 = create認証済み副担当者();

      const result = 再検討依頼取り消しユースケース(再検討中, 副担当者);
  ...
  describe("外部査読中への取り消し", () => {
    it("再検討中の資源評価を外部査読中に戻す（元ステータスから自動判定）", () => {
      const 再検討中 = create再検討中の資源評価_外部査読中から();
      const 管理者 = create認証済み資源評価管理者();

      const result = 再検討依頼取り消しユースケース(再検討中, 管理者);
