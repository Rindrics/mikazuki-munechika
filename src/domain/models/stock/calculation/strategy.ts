import type { ABC算定結果, 漁獲量データ, 生物学的データ, 資源量指標値データ } from "../../../data";
 ...
 * 資源計算結果（基底インターフェース）
 *
 * 一次処理・前進計算の出力。最終年までの年齢別資源尾数、親魚量、加入量を保持する。
 */
 ...
 * 当年までの資源計算結果
 *
 * 一次処理の出力。Branded Type により翌年資源計算結果と型レベルで区別される。
 *
 ...
 * 翌年までの資源計算結果
 *
 * 前進計算の出力。Branded Type により当年資源計算結果と型レベルで区別される。
 *
 * @see ADR 0025 for design rationale
 */
 ...
 * ABC 算定フロー:
 * 1. 一次処理: 当年までの資源計算
 * 2. 前進計算: 翌年までの資源計算（再生産関係の残差リサンプリング）
 * 3. 将来予測: 将来の資源量・漁獲量を予測
 * 4. ABC決定: 漁獲管理規則と調整係数βから ABC を決定
 *
 * @see ADR 0022 for design rationale
 */
