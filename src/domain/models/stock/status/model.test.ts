...
describe("受理", () => {
  it("内部査読中の資源評価を受理する", () => {
    ...
    const 日時 = new Date("2025-01-01T09:00:00Z");
    const { 受理済み資源評価, 受理済み } = 受理(内部査読中の資源評価, 日時, 認証済み資源評価管理者);
  ...
  it("外部査読中の資源評価を受理する", () => {
    ...
    const 日時 = new Date("2025-01-01T09:00:00Z");
    const { 受理済み資源評価, 受理済み } = 受理(外部査読中の資源評価, 日時, 認証済み資源評価管理者);
...
describe("受理取り消し", () => {
  ...
  describe("内部査読受理の取り消し", () => {
    it("資源評価のステータスを「外部公開可能」から「内部査読中」に変更する", () => {
      ...
      const { 内部査読中資源評価 } = 受理取り消し(
        内部査読受理済みの資源評価,
        日時,
        認証済み資源評価管理者
      );
      expect(内部査読中資源評価!.作業ステータス).toBe("内部査読中");
      expect(内部査読中資源評価!.対象).toEqual(内部査読受理済みの資源評価.対象);
    });
  ...
  describe("外部査読受理の取り消し", () => {
    it("資源評価のステータスを「外部査読受理済み」から「外部査読中」に変更する", () => {
      ...
      const { 外部査読中資源評価 } = 受理取り消し(
        外部査読受理済みの資源評価,
        日時,
        認証済み資源評価管理者
      );
      expect(外部査読中資源評価!.作業ステータス).toBe("外部査読中");
      expect(外部査読中資源評価!.対象).toEqual(外部査読受理済みの資源評価.対象);
    });
...
describe("再検討依頼取り消し", () => {
  describe("内部査読中への取り消し", () => {
    it("資源評価のステータスを「再検討中」から「内部査読中」に変更する", () => {
      ...
      const { 内部査読中資源評価 } = 再検討依頼取り消し内部査読中へ(
        再検討中の資源評価,
        日時,
        認証済み副担当者
      );
      expect(内部査読中資源評価.作業ステータス).toBe("内部査読中");
      expect(内部査読中資源評価.対象).toEqual(再検討中の資源評価.対象);
    });
    ...
    it("再検討依頼取り消しイベントを正しく生成する", () => {
      ...
      const { 再検討依頼取り消し済み } = 再検討依頼取り消し内部査読中へ(
        再検討中の資源評価,
        日時,
        認証済み副担当者
      );
  ...
  describe("外部査読中への取り消し", () => {
    it("資源評価のステータスを「再検討中」から「外部査読中」に変更する", () => {
      ...
      const { 外部査読中資源評価 } = 再検討依頼取り消し外部査読中へ(
        再検討中の資源評価,
        日時,
        認証済み資源評価管理者
      );
      expect(外部査読中資源評価.作業ステータス).toBe("外部査読中");
      expect(外部査読中資源評価.対象).toEqual(再検討中の資源評価.対象);
    });
    ...
    it("再検討依頼取り消しイベントを正しく生成する", () => {
      ...
      const { 再検討依頼取り消し済み } = 再検討依頼取り消し外部査読中へ(
        再検討中の資源評価,
        日時,
        認証済み資源評価管理者
      );
