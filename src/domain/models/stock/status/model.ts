...
export function 受理取り消し(
  ...
  日時: Date,
  操作者: 認証済資源評価管理者
): { 内部査読中資源評価: 内部査読中資源評価; 受理取り消し済み: 内部査読受理取り消し済み };
export function 受理取り消し(
  対象資源評価: 外部査読受理済み資源評価,
  日時: Date,
  操作者: 認証済資源評価管理者
): { 外部査読中資源評価: 外部査読中資源評価; 受理取り消し済み: 外部査読受理取り消し済み };
export function 受理取り消し(
  ...
  if (対象資源評価.作業ステータス === "外部公開可能") {
    ...
    return { 内部査読中資源評価: 更新後資源評価, 受理取り消し済み: イベント };
  } else {
    const { 更新後資源評価, イベント } = ステータス遷移(
      対象資源評価,
      日時,
    ...
    return { 外部査読中資源評価: 更新後資源評価, 受理取り消し済み: イベント };
  }
}
...
export function 再検討依頼取り消し内部査読中へ(
  対象資源評価: 再検討中資源評価,
  日時: Date,
  操作者: 認証済資源評価管理者 | 副担当者
): {
  ...
  const { 更新後資源評価: 内部査読中資源評価, イベント: 再検討依頼取り消し済み } = ステータス遷移(
    対象資源評価,
    日時,
    操作者,
    再検討依頼取り消し内部査読中へイベント定義
  );
  return { 内部査読中資源評価, 再検討依頼取り消し済み };
}

export function 再検討依頼取り消し外部査読中へ(
  対象資源評価: 再検討中資源評価,
  日時: Date,
  操作者: 認証済資源評価管理者
): {
  ...
  const { 更新後資源評価: 外部査読中資源評価, イベント: 再検討依頼取り消し済み } = ステータス遷移(
    対象資源評価,
    日時,
    操作者,
    再検討依頼取り消し外部査読中へイベント定義
  );
  return { 外部査読中資源評価, 再検討依頼取り消し済み };
}
